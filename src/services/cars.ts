...
export type TCars = {
  ...
  stockResponse: {
    ...
    searchOptions: {
      ...
      options: Record<
        string,
        Array<{ displayName: string; uriValue: string; count: string }>
      >;
    };
    sortOptions: Array<{
...
const getCars = async <T = TCars>(
  ...
  const url = `https://autotrader.co.uk/json/dealers/stock?advanced=true&advertising-location=at_cars&advertising-location=at_profile_cars&dealer=${dealer}${
...
export const allBranchesCars = async (query: Record<string, any>) => {
  ...
  const BRANCHES = getBranchesInfo();
  const branchPromises = Object.values(BRANCHES).map((branch) =>
    getCars(searchParams, branch.dealerId)
  );
  ...
  const initialResponse: TCars["stockResponse"] = {
    ...
    csFinanceProductInfoText:
      branchResults?.[0]?.stockResponse?.csFinanceProductInfoText,
    csFinanceProductInfoPageLocation:
      branchResults?.[0]?.stockResponse?.csFinanceProductInfoPageLocation,
    hpFinanceProductInfoText:
      branchResults?.[0]?.stockResponse?.hpFinanceProductInfoText,
    hpFinanceProductInfoPageLocation:
      branchResults?.[0]?.stockResponse?.hpFinanceProductInfoPageLocation,
    pcpFinanceProductInfoText:
      branchResults?.[0]?.stockResponse?.pcpFinanceProductInfoText,
    pcpFinanceProductInfoPageLocation:
      branchResults?.[0]?.stockResponse?.pcpFinanceProductInfoPageLocation,
  };
  const cars = branchResults.reduce((acc, branchResponse) => {
    acc.hasMoreResults =
      acc.hasMoreResults || branchResponse?.stockResponse?.hasMoreResults;
    acc.currentPage =
      branchResponse?.stockResponse?.currentPage || acc.currentPage;
    acc.totalResults += +branchResponse?.stockResponse?.totalResults;
    acc.totalPages = Math.max(
      acc.totalPages,
      branchResponse?.stockResponse?.totalPages
    );
    acc.isPostcodeValid =
      acc.isPostcodeValid || branchResponse?.stockResponse?.isPostcodeValid;
    acc.results = acc.results.concat(branchResponse?.stockResponse?.results);
    return acc;
  }, initialResponse);

  cars.searchOptions = branchResults
    .map((br) => br.stockResponse.searchOptions)
    .reduce(
      (mergedResult, currentOptions) => {
        const allKeys = new Set<string>([
          ...Object.keys(mergedResult.options),
          ...Object.keys(currentOptions.options),
        ]);
        allKeys.forEach((key) => {
          const combinedOptions = (mergedResult.options[key] || []).concat(
            currentOptions.options[key] || []
          );
          const optionsMap: Map<
            string,
            { displayName: string; uriValue: string; count: string }
          > = new Map();
          ...
          combinedOptions.forEach((option) => {
            ...
            if (existingOption) {
              const updatedCount =
                parseInt(existingOption.count) + parseInt(option.count);
              optionsMap.set(option.uriValue, {
                ...option,
        ...
        mergedResult.resultCount = (
          parseInt(mergedResult.resultCount) +
          parseInt(currentOptions.resultCount)
        ).toString();
        return mergedResult;
      },
      {
  ...
  const activeSort = cars?.sortOptions?.find(
    (f) => f.isSelected || f.isPreselected
  );
  if (activeSort) {
    cars.results = cars.results?.sort((a, b) => {
      if (
        activeSort?.value?.startsWith("price-") &&
        activeSort?.value?.endsWith("-asc")
      ) {
        return (
          parseNumber(a.price, ["en-us"]) - parseNumber(b.price, ["en-us"])
        );
      } else if (
        activeSort?.value?.startsWith("price-") &&
        activeSort?.value?.endsWith("-desc")
      ) {
        return (
          parseNumber(b.price, ["en-us"]) - parseNumber(a.price, ["en-us"])
        );
      } else if (
        activeSort?.value?.startsWith("year-") &&
        activeSort?.value?.endsWith("-asc")
      ) {
        return (
          +a.vehicle.yearText.slice(0, 4) - +b.vehicle.yearText.slice(0, 4)
        );
      } else if (
        activeSort?.value?.startsWith("year-") &&
        activeSort?.value?.endsWith("-desc")
        ...
        return (
          +b.vehicle.yearText.slice(0, 4) - +a.vehicle.yearText.slice(0, 4)
        );
      } else if (activeSort?.value === "mileage") {
        return (
          parseNumber(a.vehicle.mileage, ["en-us"]) -
          parseNumber(b.vehicle.mileage, ["en-us"])
        );
      }
      return 0;
    });
...
const getSingleCarDetails = async <T = TSingleCar>(advertId: string) => {
  ...
  const SellerContactLvl1 = `sellerContact {
        phoneNumberOne
        phoneNumberTwo
        protectedNumber
        byEmail
  ...
  const vehicleCondition = `vehicleCondition {
    tyreCondition
    interiorCondition
    bodyCondition
    __typename
  ...
  const reservationLvl2 = `reservation { 
    status
    eligibility
    feeCurrency
  ...
  const specificationLvl1 = `specification {
    isCrossover
    operatingType
    emissionClass
    endLayout
  ...
  const specificationLvl1Part1 = `specification{
    co2Emissions {
        co2Emission
        unit
        __typename
  ...
  const specificationLvl1Part2 = `specification{capacityHeight { 
    height
    unit 
    __typename
}
...
__typename
  ...
  const mergedData: T = lvl1.reduce((acc: any, curr: any) => {
    const advertData = curr?.[0]?.data?.search?.advert;
    if (advertData) {
      Object.keys(advertData).forEach((key) => {
        if (
          acc[key] &&
          typeof acc[key] === "object" &&
          typeof advertData[key] === "object"
        ) {
          // Deep merge objects if both the current accumulator and new value are objects
          acc[key] = deepMergeObjects(acc[key], advertData[key]);
        } else {
          // Otherwise, just set/overwrite the key in the accumulator
          acc[key] = advertData[key];
        }
      });
    }
    return acc;
